...
public class BinarySearchTree {
  ...
  private static boolean checkCompleteBinaryTree(Node root) {
	  ...
	  if(root==null)
		...
		return checkCompleteBinaryTreeUtils(root,0,n);
		
		
	}

  private static boolean checkCompleteBinaryTreeUtils(Node root, int i, int n) {
    if(root==null)
      return true;
    if(i>n)return false;
    return checkCompleteBinaryTreeUtils(root.left,i*2+1,n) && checkCompleteBinaryTreeUtils(root.right,i*2+2,n);
  }
  ...
  private static boolean isIdentical(Node root, Node root1) {
    if (root == null && root1 == null)
      return true;
    if (root != null && root1 != null)
      return root.data == root1.data && isIdentical(root.left, root1.left)
  ...
  private static int getLevel(Node root, int level, int key) {
    ...
    if (root.data == key)
      ...
      return getLevel(root.left, level + 1, key);
    else
  ...
  public static void main(String[] args) {
    ...
    System.out
        .println("%%%%%%%%%%%%%%%% print nodes k distance from root %%%%%%%%%%%%%%%%%%%%%%%%");
